...
function visual() {
  ...
  for (let i = particles.length - 1; i >= 0; i--) {
    if (!particles[i].edges()) {
      particles[i].update(amp > 160)  //Can use this to set when the particles will move
      particles[i].show()
    } else {
...
function fade_out() {     //Code for the screen that comes out
  ...
  if(timer >= 110000)     //Fade-in start
  {
    P += 0.8
    if(timer >= 115000)
    {
      resizeCanvas(windowWidth, windowHeight);      //Making new screen
      ...
      background(0, 0, 0);
      final_score();                               //Showing final score, retry button and menu button
